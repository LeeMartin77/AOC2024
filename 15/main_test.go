...
import (
	...
	"github.com/LeeMartin77/AOC2024/15/solution"
...
<`
	solution.ComputeSolutionOne([]byte(teststring))
	//.Equal(t, int64(10092), res)
}
...
v^^>>><<^^<>>^v^<v^vv<>v^<<>^<^v^v><^<<<><<^<v><v<>vv>>v><v^<vv<>v^<<^`
	...
	assert.Equal(t, int64(10092), res)
}
